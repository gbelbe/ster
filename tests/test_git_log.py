...
def _make_log_line(full: str, short: str, subject: str,
                   author: str, date: str, refs: str) -> str:
...
def test_parse_log_single_commit():
    line = _make_log_line("a" * 40, "aaaaaaa", "Initial commit",
                          "Alice", "2024-01-15", "HEAD -> main")
...
def test_parse_log_refs_stripped():
    line = _make_log_line("c" * 40, "ccccccc", "Msg", "Carol", "2024-01-10",
                          "  tag: v1.0  ")
    entries = _parse_log(line)
...
def test_fetch_diff_header_failure(tmp_path):
    with patch("ster.git_log._git", side_effect=[_completed(returncode=1),
                                                  _completed("+line\n")]):
        lines = _fetch_diff("abc1234", None, tmp_path)
...
def test_fetch_diff_diff_failure(tmp_path):
    with patch("ster.git_log._git", side_effect=[_completed("commit abc\n"),
...
def test_compute_diff_alt_label_added():
    ...
    assert diff[BASE + "C"].status == "changed"
    fd = diff[BASE + "C"].field_diffs
    assert any(f.label == "altLabel[en]" and f.before == "" and f.after == "synonym"
               for f in fd)
...
def test_compute_diff_alt_label_removed():
    ...
    assert diff[BASE + "C"].status == "changed"
    fd = diff[BASE + "C"].field_diffs
    assert any(f.label == "altLabel[en]" and f.before == "old-alt" and f.after == ""
               for f in fd)
...
def test_build_diff_taxonomy_deleted_attached_to_parent():
    ...
    parent = Concept(uri=BASE + "P", labels=[Label("en", "P", LabelType.PREF)],
                     narrower=[BASE + "C"])
    child  = Concept(uri=BASE + "C", labels=[Label("en", "C", LabelType.PREF)],
                     broader=[BASE + "P"])
...
def test_auto_fold_unchanged_subtree():
    ...
    diff = {BASE + "P": ConceptChange(BASE + "P", "unchanged"),
...
def test_viewer_load_log_with_file_path(tmp_path):
    ...
    r    = _completed(line)
    with patch("ster.git_log._git", return_value=r) as mock, \
         patch.object(GitLogViewer, "_load_diff_tree"):
...
def test_viewer_load_diff_tree_caches(tmp_path):
    ...
    fake_t  = T()
    fake_st = {}
    with patch("ster.git_log._get_file_at_commit", return_value=None), \
...
def test_launch_git_log_explicit_repo(tmp_path):
    ...
    with patch("ster.git_log.find_repo_root") as mock_find, \
...
def test_launch_git_log_file_path(tmp_path):
    ...
    fp.write_text("")
    mock_viewer = MagicMock()
    with patch("ster.git_log.find_repo_root", return_value=tmp_path), \
