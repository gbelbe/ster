...
def _make_mock_ontpub(html_content: str = "<html><body>ontology</body></html>"):
    mock_cls = MagicMock()
    mock_instance = MagicMock()
    mock_instance.make_html.return_value = html_content
    mock_cls.return_value = mock_instance
...
def test_generate_html_multilang_injects_switcher(tmp_path, ttl_file):
    if not _pylode_available():
        pytest.skip("pylode not installed")
    output_dir = tmp_path / "out"
    ...
    with patch("pylode.VocPub", mock_vp_cls):
        created = generate_html(ttl_file, output_dir, languages=["en", "fr"])
    for p in created:
        assert "ster-lang-bar" in p.read_text()


def test_generate_html_single_lang_no_switcher(tmp_path, ttl_file):
    if not _pylode_available():
        pytest.skip("pylode not installed")
    output_dir = tmp_path / "out"
    mock_vp_cls = _make_mock_vocpub("<html><body>content</body></html>")
    with patch("pylode.VocPub", mock_vp_cls):
        created = generate_html(ttl_file, output_dir, languages=["en"])
    assert "ster-lang-bar" not in created[0].read_text()


def test_generate_html_ontpub_path(tmp_path, owl_ttl_file):
    if not _pylode_available():
        pytest.skip("pylode not installed")
    output_dir = tmp_path / "out"
    mock_ont_cls = _make_mock_ontpub("<html><body>ontology docs</body></html>")
    with patch("pylode.OntPub", mock_ont_cls):
        created = generate_html(owl_ttl_file, output_dir, profile="ontpub")
    assert len(created) == 1
    assert created[0].name == "onto.html"
    assert created[0].exists()
...
def test_generate_html_defaults_to_en_when_no_labels(tmp_path):
    ...
    with patch("pylode.VocPub", mock_vp_cls):
        created = generate_html(bare_ttl, tmp_path / "out2")
    assert len(created) == 1


def test_generate_html_both_profile_falls_back_to_vocpub(tmp_path):
    """When file has both SKOS+OWL and no profile arg, vocpub is chosen."""
    if not _pylode_available():
        pytest.skip("pylode not installed")
    both_ttl = tmp_path / "both.ttl"
    both_ttl.write_text(
        "@prefix owl: <http://www.w3.org/2002/07/owl#> .\n"
        "@prefix skos: <http://www.w3.org/2004/02/skos/core#> .\n"
        "@prefix t: <https://ex.org/> .\n"
    ...
    mock_vp_cls = _make_mock_vocpub("<html><body>both</body></html>")
    with patch("pylode.VocPub", mock_vp_cls):
