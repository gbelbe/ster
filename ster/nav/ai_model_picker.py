...
_Replacer = Callable[..., AiModelPickState]
...
def _on_key_prompt(
    st: AiModelPickState, key: int, _s: _Replacer
) -> tuple[AiModelPickState, bool] | None:
    """Handle the shared text-input prompt overlay. Returns None when not active."""

    if not st.key_prompt:
        return None
    ...
    if key in (ord("\n"), ord("\r"), 343):  # Enter → confirm
        return _on_key_prompt_confirm(st, _s)
...
def _on_key_prompt_confirm(st: AiModelPickState, _s: _Replacer) -> tuple[AiModelPickState, bool]:
    """Handle Enter inside the shared text-input prompt."""
    from .. import ai
    ...
    if st.ep_field:  # editing an endpoint field
        _ep_field_map = {
            "url": lambda v: _s(
                ep_url=v, key_prompt=False, key_buffer="", key_pos=0, ep_field="", error=""
            ),
            "key": lambda v: _s(
                ep_key=v, key_prompt=False, key_buffer="", key_pos=0, ep_field="", error=""
            ),
        }
        make_new = _ep_field_map.get(st.ep_field) or (
            lambda v: _s(
                ep_model=v, key_prompt=False, key_buffer="", key_pos=0, ep_field="", error=""
            )
        )
        new = make_new(value)
...
def _on_key_model_select(
    st: AiModelPickState, mid: str, _s: _Replacer
) -> tuple[AiModelPickState, bool]:
    """Select an llm-plugin model (local or external): prompt for key if needed."""
    from .. import ai
...
def _on_key_top(st: AiModelPickState, key: int, _s: _Replacer) -> tuple[AiModelPickState, bool]:
    """Handle keys at the top level (copypaste / local / external choice)."""
    from .. import ai
    ...
    if key in (ord("\n"), ord("\r"), 343):
        ...
        if mid == "external":
            ...
            return _s(
                level="external", items=items, cursor=0, scroll=0, ep_return_items=items, error=""
            ), False
    return st, False


def _on_key_local(st: AiModelPickState, key: int, _s: _Replacer) -> tuple[AiModelPickState, bool]:
    """Handle keys at the local-model level."""
    from .. import ai
    ...
    if key in (ord("\n"), ord("\r"), 343):
        ...
        if mid.startswith("ollama:"):
            ai.save_endpoint("http://localhost:11434/v1", "", mid[7:])
            return st, True
        ...
        return _on_key_model_select(st, mid, _s)
    return st, False


def _on_key_external(
    st: AiModelPickState, key: int, _s: _Replacer
) -> tuple[AiModelPickState, bool]:
    """Handle keys at the external-model level."""
    from .. import ai
...
def _on_key_endpoint(
    st: AiModelPickState, key: int, _s: _Replacer
) -> tuple[AiModelPickState, bool]:
    """Handle keys in the custom endpoint form."""
    from .. import ai
...
_LEVEL_HANDLERS = {
    "top": _on_key_top,
    "local": _on_key_local,
    "external": _on_key_external,
    "endpoint": _on_key_endpoint,
}


def on_key(st: AiModelPickState, key: int) -> tuple[AiModelPickState, bool]:
    ...
    prompt_result = _on_key_prompt(st, key, _s)
    ...
    handler = _LEVEL_HANDLERS.get(st.level)
    if handler:
        return handler(st, key, _s)
    return st, False
