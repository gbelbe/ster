...
def search_update(state: TreeState, key: int) -> TreeState:
    ...
    if key == 27:  # Esc
        ...
        return TreeState(flat=state.flat, cursor=state.cursor, scroll=state.scroll,
                         folded=state.folded, search=new_search)

    if key in (KEY_BACKSPACE, 127, 8):
        new_query = s.query[:-1] if s.query else ""
        return _replace_search(state, SearchState(query=new_query, active=True,
                                                   matches=s.matches, current_idx=s.current_idx,
                                                   pattern=s.pattern))

    if key in (KEY_ENTER, ord("\n"), ord("\r")):
        # Commit — deactivate typing, keep highlights, open detail
        return _replace_search(state, SearchState(query=s.query, active=False,
                                                   matches=s.matches, current_idx=s.current_idx,
                                                   pattern=s.pattern))
...
def update_search_results(state: TreeState, matches: list[int],
                           pattern: re.Pattern | None) -> TreeState:
    ...
    return TreeState(
        flat=state.flat, cursor=cursor, scroll=scroll, folded=state.folded,
        search=SearchState(query=s.query, active=s.active, matches=matches,
                           current_idx=new_idx, pattern=pattern),
    )
...
def _search_jump(state: TreeState, direction: int) -> TreeState:
    ...
    return TreeState(
        flat=state.flat, cursor=cursor, scroll=scroll, folded=state.folded,
        search=SearchState(query=s.query, active=s.active, matches=s.matches,
                           current_idx=new_idx, pattern=s.pattern),
    )
